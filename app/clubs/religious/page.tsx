...
export default function ReligiousPage() {
  ...
  useEffect(() => {
    const fetchClubs = async () => {
      try {
        ...
        const religious = data
          .filter((club: { category: string }) => club.category === "religious")
          .map((club: { name: string; description: string }) => ({
            name: club.name,
            description: club.description,
            href: `/clubs/religious/${club.name.toLowerCase()}`,
          }));

        setClubs(religious);
      } catch (err) {
        console.error("Failed to load religious clubs", err);
      }
    };
  ...
  return (
    <main className="pt-2">
      <CategoryHero title="Religious Associations" image="/religion.jpg" />
